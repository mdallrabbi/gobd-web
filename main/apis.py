...
def store_task_notification(request, last_request_time):
	notification = Task.objects.get(created_at__gt=last_request_time, status=Task.PICKUP)
	...
	with transaction.atomic():
		...
		try:
			...
			task_instance = Task.objects.get(id=task_id, 
											...
											delivery_boy=None, 
											status=Task.PICKUP)
			task_instance.status = Task.RETURN
			task_instance.save()
			success_data = {
				'result': 'OK'
			}
			...
			return JsonResponse(
					{"status": "failed", 
					"error":"task has accepted by delivery boy"})
...
def delivery_boy_accept_task(request):
	...
	"""
	task_id = request.GET.get('task_id', None)
	if Task.objects.filter(delivery_boy=request.user.delivery_boy, status=Task.PICKUP).count() >= 3:
		return JsonResponse({"status": "failed", "error": "reached maximum limit"})
	...
	with transaction.atomic():
		...
		try:
			...
			task_instance.status = Task.PICKUP
			...
			return JsonResponse(success_data, status=status.HTTP_201_CREATED)
				...
				return JsonResponse({"status": "failed", "error":"order has been accepted by another delivery boy"})
...
def delivery_boy_reject_task(request):
	...
	return JsonResponse({"status": "delivery order rejected"}, status=status.HTTP_200_OK)
...
def delivery_boy_complete_task(request):
	...
	task.status = Task.DELIVERED
	task_id = task.id
	task.save()
	# deliver_task_completed_notification.delay(task_id)
	return JsonResponse({"status": "order delivered"}, status=status.HTTP_200_OK)
...
def get_deliver_boy_completed_tasks(request):
	...
	tasks = TaskSerializer(
		Task.objects.filter(status=Task.DELIVERED, delivery_boy=d_boy).order_by("-id"),
		many=True
		).data
	return JsonResponse({"tasks" : tasks})
...
def delivery_boy_ready_new_tasks(request):
	tasks = TaskSerializer(
		Task.objects.filter(status=Task.ACCEPTED, delivery_boy=None).order_by("-id"),
		many=True
		).data
	return JsonResponse({"tasks" : tasks})
...
def delivery_boy_accept_task_token(request):
	if request.method == 'POST':
		...
		if Task.objects.filter(delivery_boy=d_boy).exclude(status=Task.PICKUP):
...
def delivery_boy_reject_task_token(request):
	access_token = AccessToken.objects.get(token=request.POST.get("access_token"), expires__gt=timezone.now())
	d_boy = access_token.user.delivery_boy
	task = Task.objects.get(id=request.POST["task_id"], delivery_boy=d_boy, status=Task.PICKUP)
	task.status = Task.ACCEPTED
	# implement push notification here store manager
	task.save()
	return JsonResponse({"status": "success"})